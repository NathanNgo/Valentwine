class_name UI_animator extends Node
...
@export var wait_for: UI_animator
...
@export
var properties: Array[String] = ["scale", "position", "rotation", "size", "self_modulate"]
...
func setup() -> void:
	...
	hover_values = {
		"scale": target.scale + hover_scale,
	...
	deemphasize_values = {
		"scale": target.scale + deepmhasize_scale,
...
func on_enter(finished_waiting: bool = false) -> void:
	...
	if random_entrance:
		actual_entrance_delay = (
			rng.randf() * entrance_delay / 2 + rng.randf() * entrance_delay / 2
		)
		actual_entrance_time = (
			rng.randf() * entrance_time / 2 + rng.randf() * entrance_time / 2
		)
	if !target.visible:
		return
...
func exit() -> void:
	...
	for property in properties:
		(
			tween
			. tween_property(target, property, exit_values[property], exit_time)
			. set_trans(exit_transition)
...
func blip() -> void:
	...
	(
		tween
		. tween_property(target, "rotation", default_values["rotation"], hover_time)
...
func add_tween(
	...
	for property in properties:
		(
			tween
			. tween_property(target, property, values[property], seconds)
