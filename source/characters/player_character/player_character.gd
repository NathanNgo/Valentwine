...
func grabbed() -> void:
	...
	if (
		Input.is_action_just_pressed(selected_scheme[Controls.LEFT])
		and current_stun_prompt == Controls.LEFT
	):
	...
	if (
		Input.is_action_just_pressed(selected_scheme[Controls.RIGHT])
		and current_stun_prompt == Controls.RIGHT
	):
		escape_requirement -= 1
		current_stun_prompt = Controls.LEFT
		_left_prompt.show()
