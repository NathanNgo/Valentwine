...
func _on_trailing_line_draw_timer() -> void:
	var possible_intersection_player_one: Variant = trailing_line_one.add_point(player_one.global_position)

	if possible_intersection_player_one:
		_kill_circled_enemies(possible_intersection_player_one)
		trailing_line_one.clear_line()

	var possible_intersection_player_two: Variant = trailing_line_two.add_point(player_two.global_position)

	if possible_intersection_player_two:
		_kill_circled_enemies(possible_intersection_player_two)
		trailing_line_two.clear_line()

	trailing_line_draw_timer.start(TRAILING_LINE_DRAW_TIME)


func _kill_circled_enemies(intersection: Vector2) -> void:
	for enemy in enemies_container.get_children():
		if (
			Geometry2D.is_point_in_polygon(
				enemy.global_position,
				trailing_line_one.get_closed_polygon(
					intersection
				)
			)
		):
